...
def extract(input, t: torch.Tensor, x: torch.Tensor): # input : 타임 스케쥴, t : 배치별 타임스텝, x : 배치별 이미지
    ...
    return out.reshape(*reshape) # time step별 스케쥴링에 맞는 계수를 return
...
class BaseScheduler(nn.Module):
    ...
    def __init__(
        ...
        alphas = 1 - betas
        alphas_cumprod = torch.cumprod(alphas, dim=0) # 누적되는 알파값들
...
class DiffusionModule(nn.Module):
    ...
    def q_sample(self, x0, t, noise=None):
        ...
        # Compute xt.
        # xt​=sqrt(αˉ)t​_​x0​+sqrt(1−αˉt)​​ϵ
    ...
    def p_sample(self, xt, t):
        ...
        mean = (1.0 / torch.sqrt(alphas_t)) * (xt - eps_factor * eps_theta)

        if t.item() > 0:
            noise = torch.randn_like(xt)
            x_t_prev = mean + torch.sqrt(betas_t) * noise
        else:
            x_t_prev = mean
        #######################
        return x_t_prev
