...
def step(points, pc_labels, class_labels, model):
    ...
    Output : loss
        - loss []
        - logits [B, C, N] (C: num_class)        # B번째 sample에의 N번째 point가 class마다 가질 확률을 나타냄. cross entropy에서 softmax 되서 preds와 맞는 형태로 비교되는듯 하다.
        - preds [B, N]
    """
    ...
    B, C, N = logits.shape
    logits_for_loss = logits.permute(0,2,1).contiguous()   # [B, N, C]  # [B, N, C]

    loss = F.cross_entropy(
        logits_for_loss.view(B * N, C),
        pc_labels.view(B * N)
    )
