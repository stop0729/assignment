...
class DiffusionModule(nn.Module):
    ...
    def ddim_p_sample(self, xt, t, t_prev, eta=0.0):
        ...
        eps_theta = self.network(xt, t)
        # sigma = eta * torch.sqrt((1 - alpha_prod_t_prev) * betas_t / (1 - alpha_prod_t))
        sigma = eta * torch.sqrt(betas_t)
        mean = torch.sqrt(alpha_prod_t_prev) * (xt - torch.sqrt(1 - alpha_prod_t) * eps_theta) / torch.sqrt(alpha_prod_t) + torch.sqrt(1 - alpha_prod_t_prev - (sigma ** 2)) * eps_theta
    ...
    def ddim_p_sample_loop(self, shape, num_inference_timesteps=50, eta=0.0):
        ...
        for t, t_prev in zip(timesteps, prev_timesteps):
            ...
            t_prev = torch.tensor([t_prev], device=self.device)
