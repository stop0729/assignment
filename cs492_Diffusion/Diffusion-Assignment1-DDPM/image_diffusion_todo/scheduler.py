...
class DDPMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        alphas_cumprod_t = self._get_teeth(self.alphas_cumprod, torch.tensor([t], device=x_t.device))
        sigma_t = self._get_teeth(self.sigmas, torch.tensor([t], device=x_t.device))

        mean = (1.0 / torch.sqrt(alpha_t)) * (
            x_t - (beta_t / torch.sqrt(1.0 - alphas_cumprod_t)) * eps_theta)
